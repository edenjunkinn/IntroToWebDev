...
function countingCharacters2(stringToCount, characterToFind){
  // count the number of times a character appears in a stringToCount
  // look at each character one-be-one with a for loop
...
function countingCharacters3(str, search){
  ...
  // Example: if searching "Ohio" with 3 characters at a
  //time we want to stop at 'h'so we do not go past the end of the string
  var lastIndex = str.length - numChars;

  // This time, we are looking for a series of characters - a substring.
  for (var index = 0; index <= lastIndex; index++){
    // substring gets a part of a string from start to end lastIndex
